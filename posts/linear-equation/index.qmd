...
- A normal vector of the line $\textbf{n}=[n_x, n_y]=[a, b]$ is visualized with red
...
We can multiply or divide the line equation by any real number $k$ without changing the equation, in other words, $kax+kby+kc=0$ and $ax+by+c=0$ hold true to the same set of points. Let's use this property and divide the equation by $\sqrt{a^2+b^2}$ so that $a^2+b^2=1$ (or $\|\textbf{n}\|=1$). From now on, we assume that $\|\textbf{n}\|=1$.
...
We introduced new elements:

- A slider that defines the angle of the normal vector $\textbf{n}$
- A green point $\textbf{p}=[x, y]$
  - Try moving this point!
  - It is always on the line, i.e., $ax+by=0$ where $a=n_x$ and $b=n_y$
  - Alternatively, $\textbf{n} \cdot \textbf{p} = 0$: the dot product between the vector $\textbf{n}$ and $\textbf{p}$ is zero.
  - It follows that the vectors $\textbf{n}$ and $\textbf{p}$ are orthogonal.
- We also restricted $\textbf{n}$ so that $\|\textbf{n}\|=1$

We have learned so far that the coefficients $a$ and $b$ of the line equation represent the normal vector $\textbf{n}=[a, b]$, and we can express the line equation with the dot product $\textbf{n} \cdot \textbf{p} = 0$.

What is the meaning of the line equation when the point $\textbf{p}$ is not on the line, i.e., when $ax+by \neq 0$? **Spoiler:** the value is the signed distance from the point $\textbf{p}$ to our line. Let's understand this intuitively. 
...
The introduced 1D number line has ticks that represent the signed distances from our line. The projection of the point $\textbf{p}$ falls somewhere on that 1D number line (the dotted line segment), which corresponds to the dot product. Remember that $ax+by$ and $\textbf{n} \cdot \textbf{p}$ are the same. Play with the point $\textbf{p}$ (and with the slider) to understand when $ax+by$ is zero, positive and negative. Can you see that $ax+by$ (and $\textbf{n} \cdot \textbf{p}$) represents the signed distance between the point $\textbf{p}$ and our line?

By this point, you need to understand the simple line equation of the form $ax+by=0$. What about $ax+by+c=0$? 
Let's explore this!

Notice that $ax+by+c=0$ is almost similar to the $ax+by=0$ except the $c$ coefficient. 
We represented $ax+by$ with the dot product $\textbf{n} \cdot \textbf{p}$, then $ax+by+c$ can be represented with $\textbf{n} \cdot \textbf{p}+c$. 
First, we land on the 1D number line with $\textbf{n} \cdot \textbf{p}$, then move $c$ steps forward on that 1D number line. 
It means that our line is located at the $-c$ tick of the 1D number line, since we add $c$.
...
We introduced a slider for the coefficient $c$. Play with it, and notice the location of our line when $c=1$ and $c=-1$.

Can you see that $ax+by+c$ is still the signed distance from the point $\textbf{p}$ and our line? What does $c$ represent?

::: {.callout-note collapse="true"}
#### Explanation 

$ax+by+c$ (or $\textbf{n} \cdot \textbf{p} + c$) is the signed distance. We land on the 1D number line with $c'=\textbf{n} \cdot \textbf{p}$, then we have $c'+c$. The $c'+c$ is zero when $c'=-c$; 1 when $c'=-c+1$; -1 when $c'=-c-1$; and so on.
