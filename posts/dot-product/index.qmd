...
- The vector $\textbf{p}$ is visualized with green
- It has coordinates $\textbf{p}=[\|\textbf{a}\|\cos \theta, 0]$ 
...
- The length of the vector is the dot product, since $\|\textbf{p}\|=\|\textbf{a}\|\cos\theta=a_x=\textbf{a}\cdot\textbf{b}$ as we derived above
...
- Since the vectors $\textbf{p}$ and $\textbf{b}$ lie along the same line:
  ...
  - Note that $\|\textbf{a}\|\cos \theta$ is a real number, which means how many $\textbf{b}$`s are required to reach the vector $\textbf{p}$
