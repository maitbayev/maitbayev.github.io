...
Wait, how can the algebraic and geometric dot products be the same? 
How is $cos( \theta)$ related to such a straightforward sum of components?
...
where $\|\textbf{a}\|=\sqrt{a_x^2+a_y^2}$ is the length of the vector and $\theta$ is the angle between the vectors. We also have:

- $a_x = \|\textbf{a}\|\cos\alpha$ and $a_y=\| \textbf{a}\|\sin \alpha$ from the [polar coordinates](https://en.wikipedia.org/wiki/Polar_coordinate_system#Converting_between_polar_and_Cartesian_coordinates), where $\alpha$ is the polar angle.
...
With the above, we can simplify the explanation further by constraining $\|\textbf{b}\| = 1$. If the equivalence holds for $\|\textbf{b}\| = 1$, then it holds for any $\|k\textbf{b}\|$.
...
We arrived at a simplifed version of the problem: an arbitrary vector $\textbf{a}=[a_x, a_y]$ and a fixed vector $\textbf{b}=[1, 0]$. Let's check if the definitions agree now. The algebraic dot product is:
...
Honestly, I don't have a proof that provides good intuition at the moment for the generic version. Feel free to accept the facts and move on with your life 🙃. I will provide the best proof I have so far.

Let's now proof for any $\textbf{b}$ such that $\|\textbf{b}\|=1$.
...
We introduced **a slider** which represents the angle $\alpha$ (in degrees) by which both vectors are rotated. Feel free to play with the slider! 

We already proved for $\alpha=0$. Now, we want to prove that $\alpha > 0$ does not change the algebraic dot product, i.e., that it is invariant under rotations. To do this, we derive an alternative formula for the dot product:
...
The dot product above is derived purely from the lengths. Since the lengths remain unchanged under rotations, the algebraic dot product does as well, which completes our proof.
