...
In other words, any vector in the red circle above is orthogonal to **u**. Moreover, this circle is the largest one that can be found on the sphere. For example, compare with a smaller circle which spans non-orthogonal vectors with the same dot product:
...
In my opinion the above explanation is enough to understand the intuition. But, let's also give a formal proof for the expected value of $v_n^2$. 

We're looking for $\mathbb{E}\left[v_n^2\right]$. And we also have:

$$
\begin{align}
\mathbb{E}[v_1^2 + \cdots + v_n^2] &=\mathbb{E}[v_1^2] + \cdots + \mathbb{E}[v_n^2]\\
&= n\mathbb{E}[v_n^2] \\
&= 1
\end{align}
$$

Here we used the fact that the expected values for all components of **v** are the same and the linearity of the expectation.
