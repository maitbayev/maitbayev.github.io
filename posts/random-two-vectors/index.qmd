...
This problem is equivalent to taking two random vectors **u** and **v** on a circle of radius 1 and computing their [dot product](https://maitbayev.github.io/posts/dot-product/).
...
The dot product is [invariant under rotations](https://maitbayev.github.io/posts/dot-product/#rotational-invariance), which means that we can rotate both vectors such that the vector **u** is [0, 1]:
...
This simplifies the dot product to $\mathbf{u} \cdot \mathbf{v} = \cos (\alpha) = v_y$. The probability of being near orthogonal is pretty low in two dimensions, but this help in framing our problem for higher dimensions. 
...
We only have two the orthogonal vectors to **u**, and they are:
...
In three dimensions, we take two random vectors **u** and **v** on a unit sphere and rotate them together so that the vector **u** becomes [0, 0, 1] (the north pole):
...
The larger the circle from which we select the vector **v**, the closer it is to being orthogonal:
...
Similar to the 2D and 3D case, we can fix the first vector as $\mathbf{u}=[0, 0, \dots, 1]$. The second vector $\mathbf{v}=[v_1, v_2, \cdots, v_n]$ is randomly chosen from an N-dimensional unit sphere. Their dot product is then given by:
